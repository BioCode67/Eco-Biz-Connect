...
### **8. 구현 반영 노트 (Implementation Update)**

컨셉 단계의 비전이 실제 풀스택 구현으로 어떻게 실현됐는지 요약한다(상세는 Design 문서 8장).

* **AI 경영 분석 — 실데이터 기반:** 업로드한 매출·지출 CSV를 실제로 파싱해 매출 추세 회귀 예측(신뢰구간), 영업이익·이익률, 비용 구조 분해(재료비·인건비·임대료·공과금), 이상치 탐지를 산출. "AI 경영 분석"이 실제로 데이터를 분석하는 기능으로 구현됨.
* **ESG 상생지수 — 실신호:** 환경(공과금 비중)·사회(인건비 비중)·지배구조(매출 안정성) 신호로 산출, 점수 추이 제공.
* **블록체인 자산화·수익 분배:** STO 발행(ERC-1400)·구매·배당·ESG 앵커링을 SHA-256으로 기록하고 사용자에게 온체인 검증 조회 제공. 가스비/실네트워크 부분은 인프로세스 mock(실연동은 향후).
* **외부 연동(Bank/정부등록/KYC):** 설계 인터페이스를 따르는 mock으로 구현.
* **플랫폼:** 웹(Next.js)·앱(Expo)·백엔드(FastAPI) 3-티어, 라이트/다크 테마, 투자자·플랫폼 탄소 임팩트(tCO₂e) 환산 등 ESG 가치 강화 기능 포함.
