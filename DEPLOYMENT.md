...
## 4. 데모 데이터 (자동 — 별도 작업 불필요)

배포 시작 명령에 시드가 포함돼 있어 **첫 부팅에 데모 데이터가 자동으로 채워진다.**
(`startCommand`: `alembic upgrade head && python -m scripts.seed_demo --if-empty && uvicorn …`)
`--if-empty`는 **DB가 비어 있을 때만** 시드하고 데이터가 있으면 건너뛰므로,
Render 콜드 스타트로 재기동돼도 기존 데이터가 **초기화되지 않는다.**

자동 생성되는 시나리오: 관리자 1 · 소상공인 3 · 투자자 4 · STO 5종(LISTED) · 토큰 거래·배당·대출·ESG 리포트.
...
> 교수님께는 위 3개 계정으로 "소상공인 → 투자자 → 관리자"를 차례로 체험하시도록 안내하면 된다.
> 신규 회원가입도 정상 동작한다.
