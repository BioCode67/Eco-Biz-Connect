...
| 2 | 시스템이 역할별 회원가입 양식을 표시한다. 소상공인 양식: 이름, 이메일, 비밀번호, 전화번호, 상점명, 사업자 등록번호, 대표자명(정부 사업자 등록부 대조용). 투자자 양식: 이름, 이메일, 비밀번호, 전화번호. |
...
## **7. 구현 반영 노트 (Implementation Update)**

분석 단계의 UC·요구사항이 실제 구현에서 어떻게 충족됐는지 요약한다(상세는 Design 8장).

* **UC3~5 (업로드→AI 분석→ESG):** 업로드 CSV를 실제 파싱 → 추세 회귀 매출 예측(신뢰구간)·영업이익/이익률·비용 구조(재료비/인건비/임대료/공과금 비율)·이상치, ESG는 실데이터 신호(공과금·인건비·매출변동성)로 산출. 결과는 대시보드 + PDF 리포트로 제공. 업로드 시 확장자·크기뿐 아니라 **스키마(매출 컬럼 존재 여부)도 검증**하여, 매출 데이터가 없는 파일은 합성값으로 가장하지 않고 저장 전에 거절한다(4a 스키마 불일치, 고아 레코드 방지).
* **UC6~7 (대출 매칭·신청):** Bank API(mock)로 우대 상품 조회·신청, 비동기 심사 웹훅, 30일 중복 신청 가드, 대출 상태머신 스테퍼. 우대금리는 ESG 종합점수로 실제 차감 산출하며, 심사 웹훅은 `UnderReview`에서만 전이를 허용하고 **확정(`Approved`/`Rejected`)은 종료 상태로 고정**해 웹훅 재전송·번복으로 덮어쓰지 못하게 한다(멱등성, Design 8장 상태머신 준수).
* **UC8~12 (STO·마켓·구매·포트폴리오·거래):** ERC-1400 발행(mock), 검색·타입/CO₂ 정렬·비교(최대 3)·상세, KYC 후 구매(동적 총액·예상수익·탄소 환산·위험 고지), 포트폴리오 배분 도넛·배당 타임라인·탄소 임팩트·거래내역. **배당 분배**는 관리자 콘솔 UI에서 자산별로 실행 가능(토큰당 배당액 입력→총 분배액 실시간 계산→온체인 기록·감사로그 생성→보유 투자자 포트폴리오/배당 내역 즉시 반영). 판매 토큰이 0인 자산은 분배 버튼이 비활성화된다.
* **UC13 (관리자):** 시스템 모니터·알림 피드·거래량 차트·사용자 관리(검색)·감사 로그(검색)·STO 발행·플랫폼 탄소 임팩트.
* **비기능:** 라이트/다크 테마·반응형·접근성(ARIA/accessibilityLabel)·네트워크 에러 복구 UI·당겨서 새로고침, 온체인 무결성 검증 조회. 외부 시스템(Bank/Blockchain/정부등록/KYC)은 설계 인터페이스 mock(실연동은 향후 단계).
