...
> 로그인 화면의 **데모 계정 버튼**을 누르면 타이핑 없이 바로 체험할 수 있다.

### 평가 체험 시나리오 (3분)
1. **소상공인**으로 로그인 → "경영 데이터"에서 CSV 업로드(저장소의 `sample_business_data.csv`) → **매출 예측·영업이익/이익률·비용 구조·ESG 점수**가 실제 데이터로 산출됨 → "분석 리포트 PDF 내보내기" → "금융 상품"에서 ESG 우대 대출 신청.
2. **투자자**로 로그인 → 마켓에서 검색·CO₂ 저감순 정렬·상품 비교 → KYC 인증 → 토큰 구매 → 포트폴리오에서 **자산 배분·배당 타임라인·탄소 임팩트** 확인, 배당의 ⛓ 해시 클릭 시 **온체인 검증**.
3. **관리자**로 로그인 → 시스템 모니터·알림·거래량·플랫폼 탄소 임팩트 → STO 발행 → 사용자/감사 로그 검색.
> 우상단 ☀/🌙 토글로 다크 모드, 모바일 화면 폭에서도 정상 동작.
